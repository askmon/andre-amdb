    ...
    moduleProto.require = function(request) {
      ...
      if(existsPath === undefined) {
        ...
        if(!path.isAbsolute(request) && request.charCodeAt(0) !== CH_PERIOD) {
          ...
          if (fs.existsSync(basedRequestTs)) {
            existsPath = basedRequestTs;
          }
          else if (fs.existsSync(basedRequestJs)) {
            existsPath = basedRequestJs;
          }
          else if (fs.existsSync(basedIndexRequestTs)){
            existsPath = basedIndexRequestTs;
          }
          else if (fs.existsSync(basedIndexRequestJs)){
            existsPath = basedIndexRequestJs;
          }
          else {
            existsPath = request;
          }
